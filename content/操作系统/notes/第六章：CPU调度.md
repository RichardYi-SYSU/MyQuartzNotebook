...
> 记忆：当出现“就绪”时，代表抢占式。（抢占发生在运行->就绪、等待->就绪时）

抢占式调度优点：响应时间好，能防止进程长期霸占CPU，适合交互式系统
...
- Waiting Time：P1=0,P2=23,P3=25（计算公式：等待时间 = 完成时间 - 到达时间 - 执行时间）
- Turnaround Time：P1=24,P2=26,P3=28（计算公式：周转时间 = 完成时间 - 到达时间）
- 显然有：等待时间 = 周转时间 - 执行时间
...
- 就是：**高优先级任务反而被低优先级任务间接阻塞**，导致系统调度结果“像优先级被反过来了一样”。

典型场景：

1. 低优先级进程 **L** 先拿到一个锁或资源。
2. 高优先级进程 **H** 也需要这个锁，于是被阻塞。
3. 此时中优先级进程 **M** 开始运行。
4. 因为 **M** 优先级高于 **L**，所以 **L** 没机会继续运行，也就没法释放锁。
5. 结果是：**H 等 L，L 又被 M 抢占**，高优先级的 H 被中优先级的 M 间接拖住。
...
所以实际效果是：**中优先级任务阻塞了高优先级任务**。

### 优先级继承

- 优先级继承是优先级翻转的解决方法

- **优先级继承**的核心思想是：低优先级任务如果拿着高优先级任务需要的资源，就临时“继承”高优先级任务的优先级。
...
**让持有关键资源的低优先级任务尽快执行完临界区并释放资源，避免被中优先级任务打断。**

简单说：

H 等 L，那就先把 L 提到 H 的优先级，让 L 快点把锁还给 H。
